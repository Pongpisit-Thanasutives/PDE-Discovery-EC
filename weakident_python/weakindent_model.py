from .weakident_utils.feature_library_building import (
    build_feature_vector_tags,
    compute_test_function_para,
    build_feature_latex_tags,
)
from .weakident_utils.output_writing import write_output_tables
from .weakident_utils.calculations import (
    circshift,
    two_piece_fit_v2,
    least_square_adp,
    compute_cross_validation_error,
    compute_cross_validation_err_v2,
)
...
def weak_ident(
    ...
    use_cross_der: bool,
    tau,
) -> Tuple[np.array, np.array, pd.core.frame.DataFrame, float]:
    ...
    dictionary_list, idx_of_lhs_feature, idx_of_rhs_feature, _ = (
        build_feature_vector_tags(
            num_of_variables,
            dim_x,
            max_dx,
    ...
    idx_highly_dynamic_region = find_highly_dynamic_region(
        dictionary_list, scale_matrix, is_1d_ode, num_of_variables, dim_x, max_dx
    )
    # identify differential equation(s) from feature matrix.
    w, b, c_pred = diff_eqn_identification(
        tau,
        num_of_variables,
        dim_x,
        is_1d_ode,
        ...
        feature_matrix,
        scale_matrix,
        idx_highly_dynamic_region,
    )

    latex_tags_lhs, latex_tags_rhs = build_feature_latex_tags(
                num_of_variables, dim_x, is_1d_ode, dictionary_list, idx_of_lhs_feature
            )

    return w, b, c_pred, dictionary_list, np.array(latex_tags_lhs), np.array(latex_tags_rhs)
...
def weak_ident_pred(
    ...
    df_identified_eqns, df_coefficient_values, df_identification_errors = (
        write_output_tables(
            num_of_variables,
            true_coefficient_vector,
            dim_x,
            is_1d_ode,
            dictionary_list,
            idx_of_lhs_feature,
...
def build_feature_matrix_and_scale_matrix(
    ...
    if is_1d_ode:
        ...
        w_b_large, s_b_large = compute_feature_and_scale_matrix_pde(
            m_x, m_t, p_x, p_t, skip_x, skip_t, dict_list, u_hat, dx, dt, max_dx
        )

    return w_b_large, s_b_large


def find_highly_dynamic_region(
    dict_list: np.array,
    s_b: np.array,
    is_1d_ode: bool,
...
def diff_eqn_identification(
    ...
    """
    print(
        "The number of rows in the highly dynamic region is ",
        idx_highly_dynamic.shape[0],
    )

    scales_w_b = np.mean(np.abs(s_b_large[idx_highly_dynamic, :]), axis=0)
    scales_lhs_features, scales_rhs_features = (
        scales_w_b[lhs_ind],
        1 / scales_w_b[rhs_ind].flatten(),
    )
...
def weak_ident_feature_selection(
    w: np.array,
    b: np.array,
    b_tilda: np.array,
    w_tilda: np.array,
    ...
    for i in range(k):
        sys.stdout.write("\r")
        sys.stdout.write("[{:{}}] {:.1f}%".format("=" * i, k - 1, (100 / (k - 1) * i)))
        sys.stdout.flush()
        support = subspace_persuit(
            column_normalized_w, b_one_var / np.linalg.norm(b_one_var, 2), i
        )
        if display_tex:
            print("SP -- Sparsity level is    :", i + 1)
            print("SP -- Support (Coef index) :", support)
        c_pred = narrow_fit(
            w_nar_tilda, b_nar_tilda_one_var, support, s_rhs, s_lhs_one_var
        )
    ...
    coeff_sp = least_square_adp(w_nar_tilda[:, support_pred], b_nar_tilda_one_var) * (
        relative_scale[support_pred].reshape(-1, 1)
    )
...
def find_idx_of_interesting_feature(
    n: int, dim: int, is_1d_ode: bool, max_dx: int
) -> np.array:
    """
...
def narrow_fit(
    w: np.array, b: np.array, support: np.array, s_rhs: np.array, s_lhs: np.float64
) -> np.array:
    """
...
def compute_trim_score(
    w_column_norm: np.array, support: np.array, c_pred: np.array
) -> np.array:
    """
...
def subspace_persuit(phi: np.array, b: np.array, k: int) -> np.array:
    ...
    if is_disp:
        print(np.sort(lda))
    x = least_square_adp(
        np.matmul(np.transpose(phi_lda), phi_lda),
    ...
    for _ in range(itermax):
        ...
        phi_sga = phi[:, sga]
        x_temp = least_square_adp(
            np.matmul(np.transpose(phi_sga), phi_sga),
...
def compute_discrete_phi(m: int, d: int, p: int) -> np.array:
    ...
    for j in range(m):
        t_l[:, j] = np.power(
            1 + t[j], np.fliplr(np.arange(p - d, p + 1).reshape(1, -1)).flatten()
        )
        t_r[:, j] = np.power(
            1 - t[j], np.fliplr(np.arange(p - d, p + 1).reshape(1, -1)).flatten()
        )
    ...
    cfs[0, :] = np.block(
        [np.fliplr((t_l[0, :] * t_r[0, :]).reshape(1, -1)), t_l[0, 1:] * t_r[0, 1:]]
    )
...
def compute_feature_and_scale_matrix_ode(
    m: int, p: int, skip: int, dict_list: np.array, u_hat: np.array, dt: np.array
) -> Tuple[np.array, np.array]:
    """
...
def compute_feature_and_scale_matrix_pde(
    ...
    shrink_size = np.block(
        [np.ones((1, dim_x)) * m_x * 2, np.array([[m_t * 2]])]
    ).flatten()
    ss = np.block([np.ones((1, dim_x)) * skip_x, np.array([[skip_t]])]).flatten()
    ...
    s_b_large = compute_s_v2(w_b_large, dict_list, u_hat, fft_phis, subsampling_idx)
    return w_b_large, s_b_large


def compute_w_ode(
    dict_list: np.array, u_hat: np.array, fft_phis: np.array, subsampling_idx: np.array
) -> np.array:
    """
    Args:
        dict_list(np.ndarray): array of shape (L, n + 2). Here each row represents a feature,
    ...
    """
    print(
        "The number Nx  in the subsampled feature matrix is",
...
def conv_fft_v2(fu: np.array, fft_phi: np.array, subsampling_idx: np.array) -> np.array:
    """
...
def compute_test_funs(
    m_x: int, m_t: int, p_x: int, p_t: int, max_dx: np.array
) -> Tuple[np.array, np.array]:
    """
