...
def compute_tpr_ppv(c_true: np.array, c: np.array) -> Tuple[np.float64, np.float64]:
    """
...
def compute_err_inf(c_true: np.array, c: np.array) -> np.float64:
    ...
    if np.sum((c * c_true) != 0) == 0:
        ...
        err = np.max(np.abs(c[(c * c_true) != 0] / c_true[(c * c_true) != 0] - 1))
    return err
...
def compute_cross_validation_err_v2(
    support: np.array, w: np.array, b: np.array, iter_max=30
) -> np.float64:
    """
...
def compute_cross_validation_error(
    support: np.array, w: np.array, b: np.array, ratio=1 / 100
) -> np.float64:
    """
