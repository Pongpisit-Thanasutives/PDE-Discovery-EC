import numpy as np
import pandas as pd
from .feature_library_building import (
    build_eqn_str,
...
def write_true_and_identified_equations(
    c_true: np.array, c_pred: np.array, tags_lhs: list, tags_rhs: list
    ...
    df_eqns = pd.DataFrame(
        data={"True equation": true_eqn_str, "Predicted equation": predicted_eqn_str}
    )
...
def write_coefficient_table(
    num_of_u: int, c_true: np.array, c_pred: np.array, tags_lhs: list, tags_rhs: list
) -> pd.core.frame.DataFrame:
    """
...
def write_identification_err_table(
    c_true: np.array,
    c_pred: np.array,
    w: np.array,
    b: np.array,
    ...
    e_res = compute_err_res(w[idx_highly_dynamic, :], c_pred, b[idx_highly_dynamic])
    d_errs = {
        "$e_2$": [e2],
        "$e_{\infty}$": [e_inf],
        "$e_{res}$": [e_res],
...
def write_output_tables(
    ...
    df_eqns = write_true_and_identified_equations(
        c_true, c_pred, latex_tags_lhs, latex_tags_rhs
    )
    df_coe = write_coefficient_table(
        num_of_variables, c_true, c_pred, latex_tags_lhs, latex_tags_rhs
    )
    df_errs = write_identification_err_table(c_true, c_pred, w, b, idx_highly_dynamic)
