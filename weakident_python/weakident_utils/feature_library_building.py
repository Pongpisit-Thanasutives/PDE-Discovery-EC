...
from .calculations import find_corner_pts
...
def build_library_tags(
    ...
    alphas = build_partial_der_list(dim_x, max_dx, max_poly, use_cross_der, polys)
    dict_list = build_dictionary_list(lhs_idx, betas, alphas)
    return dict_list


def build_partial_der_list(
    dim_x: int, max_dx: int, max_poly: int, use_cross_der: bool, polys: list
) -> list:
    """
...
def build_dictionary_list(lhs_tag: np.ndarray, betas: list, alphas: list) -> np.array:
    """
...
def build_feature_vector_tags(
    ...
    dict_list = build_library_tags(n, dim, max_dx, max_poly, use_cross_der, lhs_tags)
    ...
    c_true = compute_true_coefficient_vector(
        n, true_coefficients, lhs_tags, dict_list, lhs_ind
    )
    rhs_ind = np.array([i for i in range(dict_list.shape[0]) if i not in lhs_ind])
...
def compute_true_coefficient_vector(
    n: int,
...
def build_feature_latex_tags(
    ...
    """
    if is_1d_ode:
        tags_lhs, tags_rhs = build_tag_1d_ode(num_of_variables, dict_list, lhs_ind)
    else:
        tags_lhs, tags_rhs = build_tag_de(num_of_variables, dim_x, dict_list, lhs_ind)

    return tags_lhs, tags_rhs


def build_tag_1d_ode(
    n: int, dict_list: np.ndarray, lhs_ind: np.ndarray
) -> Tuple[list, list]:
    """
...
def build_tag_de(
    n: int, dim_x: int, dict_list: np.ndarray, lhs_ind: np.ndarray
) -> Tuple[list, list]:
    """
...
def compute_test_function_para(
    u: np.array, xs: np.array, max_dx: int
    ...
    p_x = max(max_dx + 2, np.floor(np.log(tau) // np.log(1 - np.power(1 - 1 / m_x, 2))))
    if u[0].shape[0] > 1:
        m_t = min((T - 1) // 2, np.ceil(m_t))
        p_t = max(1 + 2, np.floor(np.log(tau) / np.log(1 - np.power(1 - 1 / m_t, 2))))
